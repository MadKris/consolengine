...
win2::win2() : Window(30, 30, 5, 5) {
    ...
    addControl(label2);
    slider1 = new Slider(2, 20, 2, 10, 13, 1, 3, A_NORMAL, A_STANDOUT, true, true);
    addControl(slider1);
    slider2 = new Slider(6, 24, 2, 10, 13, 15, 3, A_NORMAL, A_STANDOUT, true, true);
    addControl(slider2);
}
...
WindowProcessResult win2::processInput(int input) {
    ...
    switch (input) {
        case KEY_F(4):
            return WindowProcessResult(WINDOW_STATUS_CLOSE);
        default:
